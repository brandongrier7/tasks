...
export function toShortForm(question: Question): string {
    const sep = question.id + ": " + question.name.substring(0, 10);
    return sep;
}
...
export function toMarkdown(question: Question): string {
    const nameHash = "# " + question.name;
    let quest = question.body;
    const end = question.options.join("\n" + "- ");

    const choices = question.type;
    if (choices === "multiple_choice_question") {
        quest = quest + "\n" + "- " + end;
    }
    return nameHash + "\n" + quest;
}
